...
const tripDetail = ({ loaderData }: Route.ComponentProps) => {
    ...
    const { name, duration, itinerary, travelStyle, groupType, budget,
            ...
            <section className="container wrapper-md">
                <header>
                    ...
                    <div className="flex items-center gap-5">
                        ...
                        />
                        <InfoPill text={itinerary?.slice(0, 4).map((item) => item.location).join(', ') || ''}
                        ...
                        <ChipsDirective>
                            {pillItems.map((pill, index) => (
                                <ChipDirective
                                    ...
                                    cssClass={`${pill.bg} !font-medium !text-base !px-4`}
                                />
                            ))}
                        </ChipsDirective>
                    </ChipListComponent>
                        ...
                        <li key={index}>
                            ...
                            <ul>
                                {dayPlan.activities.map((activity, index: number) => (
                                    <li key={index}>
                                        <span className="flex-shring-0">{activity.time}</span>
                                        <p className="flex-grow p-18-semibold">{activity.description}</p>
                                    </li>
                                ))}
                            </ul>
                        </li>
                ...
                {visitTime.map((day) => (
                    <section key={day.title} className="visit">
                        <div>
                            <h3>{day.title}</h3>
                            <ul>
                                {day.items?.map((item) => (
